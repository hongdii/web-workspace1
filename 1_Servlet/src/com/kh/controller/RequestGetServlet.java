...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// toggle breakpoint -> 디버깅할 구간 지정, 토글브레이크포인트 지정후에 벌레아이콘 클릭하여 디버깅
		// TODO Auto-generated method stub
		//System.out.println("잘 실행되나??");
		/* 
		 * Get 방식으로 요청했기 때문에 doGet메소드가 호출됨
		 * 
		...
		String[] foods = request.getParameterValues("food"); // ["한식", "중식", "일식",...] // 체크한 값이 없다면 null이 넘어감(=반환값이 null임)
		...
		// 일반적인 흐름 : Servlet -> Service(뽑아낸 값들을 넘기면서 비지니스로직 실행) -> Dao 호출 - DB SQL문 실행 - 결과값 반환
		 ...
		 * 		수정된 내용을 다시 반영시키고자 한다면 서버를 재실행해야한다. 사소한 오타로 에러날수 있다.
		 */
		
		// * reponse 객체를 통해 사용자에게 html(응답화면)을 전달
		// 1) 이제부터 내가 출력할 내용은 문서형태이고 html파일 이다. 더불어서 문자셋은 utf-8이라는 것을 지정
